...
**Example:** The interval $[0,1]$ is a probability space with the normal notion of volume that is used in $\mathbb{R}$. Then there is a random variable $s(x) = 1$. It is equal to its own probability density. In general, any closed interval $[a,b]$ is a probability space, and the constant function $s(x) = \frac{1}{b-a}$ is its own probability density. 
...
$$\mathbb{E}(S) = \int_{\mathbb{R}} s(x) dx.$$
...
$$p(y_i | x_i, \beta) = \frac{e^{-\frac{(y_i - \beta \cdot x_i)^2}{2\sigma^2} }}{\sqrt{\left(2\sigma \pi \right)}}$$
...
$$
L(\beta) := P(y_i, x_i, \beta) = \prod_i p(y_i | x_i, \beta) = \prod_i \frac{e^{-\frac{(y_i - \beta \cdot x_i)^2}{2\sigma^2} }}{\sqrt{\left(2\sigma \pi \right)}}
$$
